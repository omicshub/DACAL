...
class MultimodalDataset(Dataset):
    ...
    def __init__(self, task, data_dir, subset=0, split="train", comb=None, train_ratio=None):
        ...
        cfg_task = task.replace("_atlas", "").replace("_generalize", "").replace("_transfer", "")
        config = utils.load_toml("configs/data.toml")[cfg_task]
        ...
        for m in self.comb:
            ...
            #     self.masks[m] = np.array(mask).astype(np.float32)
    ...
    def __getitem__(self, index):
        ...
        for m, v in self.s_subset.items():
            items["s"][m] = np.array([v]).astype(np.int64)
