...
preprocess <- function(output_dir, atac = NULL, rna = NULL, adt = NULL) {
    ...
    #            ScaleData()
    #     SaveH5Seurat(rna, pj(output_dir, "rnanorm.h5seurat"), overwrite = TRUE)
    # }
    if (!is.null(rna)) {
        rna <- FindVariableFeatures(rna,nfeatures = 4000) %>%
               ScaleData()
        SaveH5Seurat(rna, pj(output_dir, "rnanonormalize11.h5seurat"), overwrite = TRUE)
    }
